...
void LCNInfoDlg::clickGet()
{
    ...
    {
        QString strFile = berApplet->findFile( this, JS_FILE_TYPE_LCN, "" );
    ...
    else
    {
        ...
        {
            berApplet->warningBox( tr("Please enter an email"), this );
            mEmailText->setFocus();
            return;
        }
...
void LCNInfoDlg::clickFree()
{
    ...
    if( ret == 0 )
    {
        if( berApplet->yesOrNoBox(tr("A free license has been issued. Restart to apply it?"), this, true))
            berApplet->restartApp();
